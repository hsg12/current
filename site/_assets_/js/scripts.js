...
(function($) {
	...
	*/
	...
	$translateButton.on('keydown click', function(e){
		if (e.keyCode === 13 || e.type === 'click') {
			if($('#translation-links ul:visible').length == 0){
				$('#header-top').css('z-index', 170);
			} else {
				$('#header-top').css('z-index', 100);
			}

			$('#translation-links ul').stop().fadeToggle(0);
	...
	const headerHeight = $('#header-bottom').height();
		...
		if (mediaQuery.matches) {
			...
			headerLi.hover(
				function() {
					...
					const bottomEdgeGap = 65;
					const maxHeight = $window.height() - headerHeight - bottomEdgeGap;
	...
	// 
	$('#nav > li > a:first-child, #nav > li > span').wrapInner('<div class="nav-link-inner"></div>');
