...
(function($) {
	...
	$("#nav-toggle").on("click", function(){
		$(".header-center-box").stop(true, true).slideToggle().css('display', 'flex');
	...
	*/
	...
	if (typeof tns !== "undefined") {
		...
		$('.shows-slider').each(function(i, el) {
			var $el = $(el);
			...
			tns({
				...
				speed: 600,
				lazyload: true,
				lazyloadSelector: '.show-item-banner, .show-item-content-banner',
				controlsText: [
				...
				responsive: {
					...
					768: { items: Math.min(2, quickLinksCount) },
					992: { items: Math.min(3, quickLinksCount) },
				}
			});
		});
	...
	// Youtube video
	if(typeof $.fn.YTPlayer !== "undefined" && $body.attr('id') === 'homepage'){
	...
	// Shows section
	$('.show-item-wrap').on('click', function() {
		const showItem = $(this).find('.show-item');
		const href = showItem.data('href');
		const target = showItem.data('target') || '_self';
	...
	$('.show-item-wrap a').on('click', function(e) {
		e.stopPropagation();
	});
	...
	$('.show-item').hover(
		function() {
			const $this = $(this);
			$this.data('hoverTimeout', setTimeout(() => {
				$this.find('.show-item-content').css('overflow', 'auto');
			}, 300));
		},
		function() {
			const $this = $(this);
			clearTimeout($this.data('hoverTimeout'));
			$this.find('.show-item-content').css('overflow', 'hidden');
		}
	);
	...
	scrollDownBtn.on('click', function() {
		...
		const offset = $window.width() < 992 ? mobilePoint : point;
		$('html, body').animate({
